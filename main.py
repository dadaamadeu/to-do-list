...
def adicionar_item_tarefa(tarefa):
    ...
    check_button = ttk.Checkbutton(frame_tarefa, command=lambda label=label_tarefa: alternar_sublinhado(label))
...
icon_path = os.path.join(current_dir, "edit.png")
icon_editar = PhotoImage(file=icon_path).subsample(3, 3)

current_dir = os.path.dirname(os.path.abspath(__file__))
icon_path = os.path.join(current_dir, "delete.png")
icon_deletar = PhotoImage(file=icon_path).subsample(3, 3)
    
fonte_cabecalho = font.Font(family="Garamond", size=24, weight="bold")
rotulo_cabecalho = tk.Label(janela, text="Minha Lista", font=fonte_cabecalho, bg="#f0f0f0", fg="#333").pack(pady=20)
...
# criar um frmae para a lista de tarefas com rolagem
